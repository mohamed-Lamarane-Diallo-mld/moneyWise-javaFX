...
public class TransactionController implements Initializable {
    ...
    private void chargerSolde() {
        if (soldeLabel == null || currentUserId == -1)
            return;
    ...
    private void construireNumeroPages(int totalPages) {
        ...
        for (int i = debut; i <= fin; i++) {
            ...
            if (i == pageActuelle) {
                btn.setStyle("-fx-background-color:#6C63FF;-fx-text-fill:white;-fx-font-size:13px;-fx-font-weight:bold;-fx-background-radius:8;-fx-cursor:hand;");
            } else {
                btn.setStyle("-fx-background-color:#F7FAFC;-fx-border-color:#E2E8F0;-fx-border-radius:8;-fx-background-radius:8;-fx-font-size:13px;-fx-text-fill:#4A5568;-fx-cursor:hand;");
            }
    ...
    private VBox creerBudgetItem(Budget budget, double pct) {
        ...
        String couleur = pct >= 100 ? "#EF4444" : (pct >= 80 ? "#F59E0B" : "#10B981");
    ...
    private HBox creerCategorieItem(Categorie cat) {
        ...
        Button btnEdit = new Button("✎ Modifier");
        btnEdit.getStyleClass().addAll("btn-action-icon", "btn-action-edit");
    ...
    private void initFiltres() {
        ...
        filterCategorie.valueProperty().addListener((obs, old, val) -> appliquerFiltres());
    ...
    private void supprimerBudget(Budget b) {
        if (AlerteHelper.confirmer("Supprimer", "Supprimer le budget \"" + b.getCategorieNom() + "\" ?")) {
            if (budgetDAO.supprimer(b.getId(), currentUserId)) rafraichirBudgets();
            else AlerteHelper.erreur("Erreur", "Impossible de supprimer.");
        }
    }
    ...
    private void supprimerCategorie(Categorie c) {
        if (AlerteHelper.confirmer("Supprimer", "Supprimer \"" + c.getNom() + "\" ?\n\n⚠ Impossible si liée à des transactions.")) {
            ...
            else AlerteHelper.erreur("Suppression impossible", "Cette catégorie est utilisée par des transactions.");
        }
    }
}