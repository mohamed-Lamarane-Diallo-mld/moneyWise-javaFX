...
public class TransactionController implements Initializable {
    ...
    private void chargerSolde() {
        if (soldeLabel == null) return;
        double solde = transactionDAO.getSoldeTotal(currentUserId);
    ...
    private void chargerResume() {
        ...
        double entrees = transactionDAO.getTotalEntreesMois(currentUserId);
        double sorties = transactionDAO.getTotalSortiesMois(currentUserId);
        double epargne = entrees - sorties;
        int nbTrans = transactionDAO.findByUtilisateur(currentUserId).size();
    ...
    @FXML private void onPageSizeChanged() {
    ...
    @FXML private void goToPremierePage() { pageActuelle = 0; afficherPageTransactions(); }
    @FXML private void goToDernierePage() { pageActuelle = getTotalPages() - 1; afficherPageTransactions(); }
    @FXML private void goToPagePrecedente() { if (pageActuelle > 0) { pageActuelle--; afficherPageTransactions(); } }
    @FXML private void goToPageSuivante() { if (pageActuelle < getTotalPages() - 1) { pageActuelle++; afficherPageTransactions(); } }
    ...
    public void rafraichirBudgets() {
        ...
        tousLesBudgets.sort((b1, b2) -> {
            ...
            if (b1EstCourant && !b2EstCourant) return -1;
            if (!b1EstCourant && b2EstCourant) return 1;
            if (b1.getAnnee() != b2.getAnnee()) return Integer.compare(b1.getAnnee(), b2.getAnnee());
    ...
    @FXML private void budgetPagePrev() { if (budgetPage > 0) { budgetPage--; afficherPageBudgets(); } }
    @FXML private void budgetPageNext() { int tp = (int) Math.ceil((double) tousLesBudgets.size() / BUDGET_PAR_PAGE); if (budgetPage < tp - 1) { budgetPage++; afficherPageBudgets(); } }
    ...
    @FXML private void toggleBudgetVisible() { budgetVisible = !budgetVisible; mettreAJourTotalBudget(); }

    private String getMoisLabel(int mois, int annee) {
        String[] moisNom = { "", "Jan", "Fév", "Mar", "Avr", "Mai", "Juin", "Juil", "Août", "Sep", "Oct", "Nov", "Déc" };
        int moisCourant = LocalDate.now().getMonthValue();
        int anneeCourante = LocalDate.now().getYear();
        if (mois == moisCourant && annee == anneeCourante) return "ce mois-ci";
        else if (mois >= 1 && mois <= 12) return moisNom[mois] + " " + annee;
        return mois + "/" + annee;
    }
    ...
    private VBox creerBudgetItem(Budget budget, double pct) {
        ...
        Label montant = new Label(NF.format(depense) + " / " + NF.format(budget.getMontantMax()) + " FCFA");
        ...
        String couleur;
        if (pct >= 100) couleur = "#EF4444";
        else if (pct >= 80) couleur = "#F59E0B";
        else couleur = "#10B981";
        ...
        pctLabel.setStyle("-fx-text-fill:" + couleur + "; -fx-font-weight:bold;");
    ...
    private void afficherPageCategories() {
        ...
        for (int i = debut; i < fin; i++) {
            categorieListContainer.getChildren().add(creerCategorieItem(toutesLesCategories.get(i)));
        }
    ...
    @FXML private void categoriePagePrev() { if (categoriePage > 0) { categoriePage--; afficherPageCategories(); } }
    @FXML private void categoriePageNext() { int tp = (int) Math.ceil((double) toutesLesCategories.size() / CATEGORIE_PAR_PAGE); if (categoriePage < tp - 1) { categoriePage++; afficherPageCategories(); } }

    private HBox creerCategorieItem(Categorie cat) {
        HBox item = new HBox(10);
        item.getStyleClass().add("categorie-item");
        item.setAlignment(Pos.CENTER_LEFT);
        Label nom = new Label((cat.getIcone() != null ? cat.getIcone() + "  " : "") + cat.getNom());
    ...
    private void appliquerFiltres() {
        ...
        transactionsFiltrees = transactionDAO.rechercher(currentUserId, typeParam, catId, debut, fin, motCle.isEmpty() ? null : motCle);
    ...
    private void initTable() {
        ...
        colActions.setCellFactory(col -> new TableCell<>() {
            ...
            { btnEdit.getStyleClass().add("btn-action-edit"); btnDelete.getStyleClass().add("btn-action-delete"); box.setAlignment(Pos.CENTER); }
            @Override protected void updateItem(Void item, boolean empty) {
                super.updateItem(item, empty);
                if (empty) { setGraphic(null); }
                else {
                    Transaction t = getTableView().getItems().get(getIndex());
                    btnEdit.setOnAction(e -> ouvrirModal(t));
    ...
    private void confirmerSuppression(Transaction t) {
        String msg = "Supprimer : " + (t.getDescription() != null ? t.getDescription() : "—") + " — " + NF.format(t.getMontant()) + " FCFA\n\nIrréversible.";
    ...
    private void ouvrirModalBudget(Budget b) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/com/project/view/BudgetModal.fxml"));
            Parent root = loader.load();
    ...
    private void supprimerBudget(Budget b) {
        if (AlerteHelper.confirmer("Supprimer", "Supprimer le budget \"" + b.getCategorieNom() + "\" ?")) {
    ...
    private void ouvrirModalCategorie(Categorie c) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/com/project/view/CategorieModal.fxml"));
            Parent root = loader.load();
    ...
    private void supprimerCategorie(Categorie c) {
        if (AlerteHelper.confirmer("Supprimer", "Supprimer \"" + c.getNom() + "\" ?\n\n⚠ Impossible si liée à des transactions.")) {
            if (categorieDAO.supprimer(c.getId())) {
                rafraichirCategories();
            } else {
                AlerteHelper.erreur("Suppression impossible", "Cette catégorie est utilisée par des transactions.");
            }
        }
    }
}